...
export const getAllTheatresForAdmin = async () => {
  try{
      const response = await axiosInstances.get('/api/theatres/get-all-theatres');
      return response.data;
  }catch(err){
      return err.response;
  }
...
export const getAllTheatres = async (payload) => {
  try{
      const response = await axiosInstances.post('/api/theatres/get-all-theatres-by-owner', payload);
      return response.data;
  }catch(err){
      return err.response;
  }
